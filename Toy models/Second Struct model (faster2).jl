...
begin
	...
	function CV(S::Grid ;vis::Bool=false,hist::Tuple{Vector{Matrix{Vector{Float64}}}, Int}=([[[[0.0]] [[0.0]]]],0)) #returns the specific heat capacity of the grid
		T, red = hist
		if vis
			cs = [S.β^2*(1/2*mean([(-S.J.*dot(neighbours!(S,i,j),t[i,j]))^2 for i in 1:S.m, j in 1:S.n])-(1/2*mean([(-S.J.*dot(neighbours!(S,i,j),t[i,j])) for i in 1:S.m, j in 1:S.n]))^2)  for t in T] #calculate CV of each grid
...
begin
    ...
    @threads for idx in 1:n
        i = betas[idx]
        g = Grid(m=100, n=100, d=1, β=i)
        evolve(g, niters=10_000_000)
        E[idx] = av_energy(g)
        M[idx] = av_mag(g)
        C[idx] = CV(g)
        X[idx] = χ(g)
...
scatter(Temps,E,xlabel="T/k_B",ylabel="E",legend=false)

# ╔═╡ 44c5f1cb-aa16-4774-88b8-929a029c06aa
scatter(Temps,M,xlabel="T/k_B",ylabel="M",legend=false)
...
scatter(Temps,X,xlabel="T/k_B",ylabel="X",legend=false)
