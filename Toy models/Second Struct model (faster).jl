...
begin
	@show VERSION
	using Plots, LinearAlgebra, Distributions, Parameters, BenchmarkTools, PlutoUI, Statistics
end
...
begin
	...
	function neighbours!(sum_vec::Vector{Float64}, S::Grid, i::Int, j::Int) # obtains the sum of the neighbours for the energy calculation
		...
		sum_vec .= NN .+ SS .+ WW .+ EE .+ NW .+ SW # return sum of the neighbours
	end
	...
	function evolve(S::Grid; niters::Int=1_000_000, store::Bool=false, red::Int=10_000) # evolves the grid for niters iterations using the Glauber or Boltzmann prob and stores them with the appropriate point reduction if needed
	    ...
	    sum_vec = zeros(Float64, S.d)  # Preallocate sum vector
	    for iter in 1:niters
	        i, j = rand(1:S.m), rand(1:S.n) #choose spin
	        sf = spin(S.d) #choose random orientation
	        neighbours!(sum_vec, S, i, j)  # Use in-place operation
	        ΔE = S.J * dot(sum_vec, (S.s[i, j] - sf))
	...
	function av_energy(S::Grid; vis::Bool=false, hist::Tuple{Vector{Matrix{Vector{Float64}}}, Int}=([[[[0.0]] [[0.0]]]],0)) #returns the average energy of the grid with the option to dsiplay its evolution over the iterations
		...
		sum_vec = zeros(Float64, S.d)  # Preallocate sum vector
	    if vis
	        energies = [1/2 * mean([-S.J * dot(neighbours!(sum_vec, S, i, j), t[i, 			j]) for i in 1:S.m, j in 1:S.n]) for t in T]
	        plot(energies, xlabel="N of iterations / $red", ylabel="Av. E", 				legend=false, title="Average Energy over each iteration")
	    else
	        return 1/2 * mean([-S.J * dot(neighbours!(sum_vec, S, i, j), S.s[i, j]) 		for i in 1:S.m, j in 1:S.n])
	    end
	end
	...
	function CV(S::Grid ;vis::Bool=false,hist::Tuple{Vector{Matrix{Vector{Float64}}}, Int}=([[[[0.0]] [[0.0]]]],0)) #returns the specific heat capacity of the grid
		...
		sum_vec = zeros(Float64, S.d)  # Preallocate sum vector
		if vis
			plot([S.β^2*(1/2*mean([(-S.J.*dot(neighbours!(sum_vec,S,i,j),t[i,j]))^2 for i in 1:S.m, j in 1:S.n])-1/2*mean([(-S.J.*dot(neighbours!(sum_vec,S,i,j),t[i,j])) for i in 1:S.m, j in 1:S.n])^2)  for t in T],xlabel="N of iterations/$(red)",ylabel="CV/k_B",legend=false,title="Heat capacity over each iteration")
		else
			S.β^2*(1/2*mean([(-S.J.*dot(neighbours!(sum_vec,S,i,j),S.s[i,j]))^2 for i in 1:S.m, j in 1:S.n])-av_energy(S)^2) 
		end
	end
...
#=╠═╡
plot(betas,E,xlabel="β",ylabel="E",legend=false)
  ╠═╡ =#
...
# ╔═╡ bda61c54-3237-49ed-9369-576be96a2e25
md"# Misc"

# ╔═╡ 77fc22d1-93af-4039-9cc1-46dc829a39bf
begin
	function neighbours_old(S::Grid,i::Int,j::Int) #obtains the sum of the neighbours for the energy calculation
			#Get the sum of neighbours of s[i,j], periodic boundary conditions
	...
	function evolve_old(S::Grid ; niters::Float64=1e7,store::Bool=false,red::Float64=1e4) #evolves the grid for niters iterations using the Glauber or Boltzmann prob and stores them with the appropriate point reduction if needed
			if store  
	...
	function av_energy_old(S::Grid ;vis::Bool=false,hist::Tuple{Vector{Matrix{Vector{Float64}}}, Int}=([[[[0.0]] [[0.0]]]],0.0)) #returns the average energy of the grid with the option to dsiplay its evolution over the iterations
		T, red = hist
		if vis
...
manifest_format = "2.0"
project_hash = "ea4fce5e7213fa53d98e0e8316ee2bde1867ed98"
